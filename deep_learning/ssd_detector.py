"""
SSD300 (VGG-16 backbone) detector, evaluated on VOC 2007 test.
...
def load_model() -> torch.nn.Module:
    """Load SSD300 VGG-16 with COCO weights."""
    from torchvision.models.detection import (
        ssd300_vgg16,
        SSD300_VGG16_Weights,
    )
    print("    Loading SSD300 weights (COCO) ...")
...
def run_inference_on_test(
    ...
    """
    Run SSD300 on every VOC 2007 test image.
    ...
    print(f"  Running SSD300 on {len(test_ids)} test images ...")
    ...
    print(f"  SSD300 done: {total_dets} detections, avg {avg_ms:.1f} ms/image")
